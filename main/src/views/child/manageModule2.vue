...
const url = `${config.manageModule2}/manageModule2/`
let microAppData = { basePath: '/app-manageModule2', msg: '来自基座的数据' }
function handleCreate() {
  console.log('app-manageModule2 创建了')
  store.dispatch('app/setAppLoading', true)
}

function handleBeforeMount() {
  console.log('app-manageModule2 即将被渲染')
}

function handleMount() {
  console.log('app-manageModule2 已经渲染完成')
  store.dispatch('app/setAppLoading', false)
}

function handleUnmount() {
  console.log('app-manageModule2 卸载了')
  store.dispatch('app/setAppLoading', false)
}

function handleError() {
  console.log('app-manageModule2 加载出错了')
  store.dispatch('app/setAppLoading', false)
}

function handleDataChange(e) {
  // 接收子应用传递的数据 e.detail.data
  console.log('来自子应用 app-manageModule2 的数据:', e.detail.data)
